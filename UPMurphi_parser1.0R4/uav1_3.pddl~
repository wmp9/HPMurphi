(define (domain UAV)
  ...
  (:requirements :typing :durative-actions :fluents :timed-initial-literals :time :conditional-effects :equality :negative-preconditions) 
  (:types pattern waypoint orientation)
  (:predicates (updated ?p - pattern ?o - orientation) (at ?p - waypoint) (active ?p - pattern) (beginAt ?w - waypoint  ?p - pattern) (endAt ?w - waypoint ?p - pattern) (oriented ?p - pattern ?o - orientation) (searched ?p - pattern))
  (:functions (reward) (rewardof ?p - pattern) (timefor ?p - pattern) (distance ?w1 ?w2 - waypoint) (penalty ?p - pattern) (award ?p - pattern))
...
(:domain UAV)
(:objects spiral1 spiral2 smallLawn6 - pattern
          origin s1s s1e s2s s2e sl6s sl6e  - waypoint
