...
extern string mantissa, exponent;
...
public:
	...
	static string print_licence() {
		...
		toReturn
				+= "Bugs, questions, and comments should be directed to \"fabio.mercorio@univaq.it\" \n\n";
...
public:
	...
	;
	virtual string toMurphi(int indent) {
		return "TIMED INITIAL LITERALS TO DO";
	}
	;
};
...
public:
	...
	;
	virtual string toMurphi(int indent) {
		return "TO DO toMurphi di derivation_rule\n";
	}
	...
	virtual string apply_continuous_change_statement() {
		...
		for (; it != operator_vector.end(); ++it) {
			...
			if (p != 0 || d != 0) { // pick a process
				...
				unsigned int pos = inside_while.find("[[DA_CONDITION]]");
