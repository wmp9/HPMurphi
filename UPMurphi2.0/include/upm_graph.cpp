...
void GraphManager::inc_outgoing_num(unsigned long from) {
	++out_degree[from];
	//fprintf(stderr,"inc_outgoing_num(%lu) -> %lu\n",from,out_degree[from]);
}
...
{
	//fprintf(stderr,"creazione archi per %lu stati\n",num_states);
	for(unsigned long i = 0; i< num_states; ++i) {
		//fprintf(stderr,"stato %lu: creazione di %lu archi uscenti...",i,out_degree[i]);
		outgoing[i] = new edge[out_degree[i]];		
MEMTRACKALLOC	
		out_degree[i] = 0;		
		//fprintf(stderr,"OK. memoria in uso: %lu bytes\n",bytes);
	}
	num_transitions = 0;
}
...
void DiskGraphManager::setup_outgoing_lists() 
{
	...
	//fprintf(stderr,"scrittura out degrees per %lu stati...\n",num_states);
	//write out degrees
	for(i = 0; i< num_states; ++i) {
		fwrite(&out_degree[i],sizeof(RULE_INDEX_TYPE),1,graph);
	}
	
	//fprintf(stderr,"creazione archi vuoti...\n",num_states);	
	//write empty transitions (pad space)
	for(i = 0; i< num_states; ++i) {
		out_position[i] = pos;
		//fprintf(stderr,"stato %lu: creazione di %lu archi uscenti dall'offset %lu\n",i,out_degree[i],out_position[i]);
