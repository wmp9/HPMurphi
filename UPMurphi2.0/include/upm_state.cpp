...
:  max_active_states(mas)
{
  ...
  cout << "First state is at " << &(stateArray[0].s) << std::endl;
MEMTRACKALLOC
...
void state_queue::Print(void)
{
  ...
  for (i = 0; i < num_elts_head; i++) {
	...
	StateCopy(workingstate, &stateArray[head_begin+front + i].s);

    // convert to print in unsigned long format?
    cout << "HEAD: State " << i << " [" << head_begin+front + i << "]:\n";
    workingstate->print();
    cout << stateArray[head_begin +front + 1].i << endl;

//    state * temp_p = new state;
//    state * temp_prev= new state;
//
//if (workingstate->previous.get_sp() != 0){
//    StateCopy(temp_p, workingstate);
//    StateCopy(temp_prev, workingstate->previous.get_sp());
//
//    StateCopy(workingstate, temp_prev);
//
//    cout << "\nPREVIOUS" << endl;
//    workingstate->print();
//    cout << " " << endl;
//
//    StateCopy(workingstate, temp_p);
//}
//else {
//	cout << "NULL PREVIOUS" << endl;
//}
  ...
  for (i = 0; i < num_elts_tail; i++) {
	  ...
	  StateCopy(workingstate, &stateArray[tail_begin + i].s);

    // convert to print in unsigned long format?
    cout << "TAIL: State " << i << " [" << tail_begin + i << "]:\n";
    workingstate->print();
    cout << stateArray[tail_begin + 1].i << endl;
...
#ifndef SPLITFILE
  global_rear +=
      fwrite(&stateArray[tail_begin], sizeof(state_and_index), tail_size,
	     paging_file_bottom);

#else
  global_rear +=
      paging_file_bottom->write(&stateArray[tail_begin], sizeof(state_and_index),
				tail_size);
#endif
...
{
#ifndef SPLITFILE
  size_t read = fread(&stateArray[head_begin], sizeof(state_and_index), head_size,
		      paging_file_top);
#else
  size_t read =
      paging_file_top->read(&stateArray[head_begin], sizeof(state_and_index),
			    head_size);
#endif 
...
int retVal;
  ...
  retVal = args->num_bits.value;
	...
	retVal += 8 * sizeof(unsigned long); //gdp disk indexes
