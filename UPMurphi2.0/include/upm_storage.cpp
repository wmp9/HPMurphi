...
StorageManager::StorageManager() : 
	transitions(NULL), goals(NULL), reachables(NULL), actions(NULL), plans(false), starts(false), errors(NULL),graph(NULL),
...
void OutputManager::WritePlans() {
	...
	int rule_duration;
	RULE_INDEX_TYPE rule;
	unsigned time, weight, nplan=0;
	unsigned temp,dur,iplan,lplan,max_lplan=0;
	edge* plan;
	
	fseek(plans,0,SEEK_SET);
	//ancora una volta prescansioniamo per risparmiare spazio, ma sarebbe molto pi� comodo scrivercelo
	...
	while(!feof(plans) && (fread(&from,sizeof(unsigned long),1,plans)==1)) {
		...
		for(iplan=1;iplan<lplan;++iplan) { 
			if (Rules->RulePDDLClass(plan[iplan].rule) == RuleManager::Action) {
				fprintf(target,"(%03d) %s [%03d]",time,Rules->RulePDDLName(plan[iplan].rule),
#ifdef VARIABLE_DURATION
						plan[iplan].duration
#else
				...
				for (dur=1,temp=iplan+1; temp<lplan; ++temp) {
				...
				fprintf(target,"(%03d) %s [%03d]",time,Rules->RulePDDLName(plan[iplan].rule),dur);				
				if(args->output_fmt.mode == argoutput_fmt::PDDL_Verbose || args->output_fmt.mode == argoutput_fmt::PDDL_VeryVerbose) {
					fprintf(target,"; weight = %03d",
#ifdef VARIABLE_WEIGHT	
						plan[iplan].weight
				...
				fprintf(target,"; (%03d) %s: %s [%03d]; weight = %03d\n",time,Rules->RulePDDLClassName(plan[iplan].rule),Rules->RulePDDLName(plan[iplan].rule),
#ifdef VARIABLE_DURATION
						plan[iplan].duration
#else
						Rules->RuleDuration(plan[iplan].rule)
			...
			//fprintf(target,"--Next state (%07ld)----------------\n",plan[iplan].to);
			//DumpState(plan[iplan].to);
		}
		fprintf(target,"; ---------------------------------------\n");		
