...
public:
  ...
  virtual const char *value() { return argv[ index ]; };
  virtual const char *nextvalue() { if (index+1>=argc) return ""; else return argv[ index+1 ]; };
...
public:
  ...
  virtual const char *value() { return current; };
...
public:
  ...
  ~argsearch_alg() {}
  void set(SearchAlgorithmtype t) { if (!initialized)  { initialized = TRUE; mode = t; }
...
public:
  ...
  { if (!initialized)
      { initialized = TRUE; doublevalue = val; }
...
private:
  const char * name;
public:
  argstring(const char* val, const char * n) : initialized(FALSE), name(n) {set(val); initialized=FALSE;};
  ~argstring() { if (initialized) delete[] value; };
