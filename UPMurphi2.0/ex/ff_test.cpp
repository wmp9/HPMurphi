/******************************
  Program "ff_test.m" compiled by "Universal Planner Murphi Release 2.0.R8"

  Murphi Last Compiled date: "Apr 23 2014"
 ******************************/
...
#define MURPHI_DATE "Apr 23 2014"
#define PROTOCOL_NAME "ff_test"
#define BITS_IN_WORLD 45
#define HASHC
...
mu_0_boolean mu_x("x",0);

/*** Variable declaration ***/
mu_0_boolean mu_y("y",2);

/*** Variable declaration ***/
mu_0_boolean mu_z("z",4);

/*** Variable declaration ***/
mu_0_boolean mu_p("p",6);

/*** Variable declaration ***/
mu_0_boolean mu_q("q",8);

/*** Variable declaration ***/
mu_0_boolean mu_r("r",10);

/*** Variable declaration ***/
mu_1_integer mu_h_n("h_n",12);

/*** Variable declaration ***/
mu_1_integer mu_g_n("g_n",23);

/*** Variable declaration ***/
mu_1_integer mu_f_n("f_n",34);
...
std::vector<mu_0_boolean*> world_class::get_mu_bools()
{
      ...
      awesome.push_back(&(mu_x));
      awesome.push_back(&(mu_y));
      awesome.push_back(&(mu_z));
      awesome.push_back(&(mu_p));
      awesome.push_back(&(mu_q));
      awesome.push_back(&(mu_r));
    return awesome; 
}
...
void world_class::print(FILE *target, const char *separator)
{
  ...
  mu_y.print(target, separator);
  mu_z.print(target, separator);
  mu_p.print(target, separator);
  mu_q.print(target, separator);
  mu_r.print(target, separator);
...
void world_class::print_diff(state *prevstate, FILE *target, const char *separator)
{
  if ( prevstate != NULL )
  {
    ...
    mu_y.print_diff(prevstate,target,separator);
    mu_z.print_diff(prevstate,target,separator);
    mu_p.print_diff(prevstate,target,separator);
    mu_q.print_diff(prevstate,target,separator);
    mu_r.print_diff(prevstate,target,separator);
...
/********************
  Goal records
 ********************/
int mu__goal_12() // Goal "enjoy"
{
return (mu_x) == (mu_true);
};

bool mu__condition_13() // Condition for Rule "enjoy"
{
  return mu__goal_12( );
}
