/******************************
  Program "bw6.m" compiled by "Universal Planner Murphi Release 2.0.R8"

  Murphi Last Compiled date: "Oct 20 2014"
 ******************************/
...
#define MURPHI_DATE "Oct 20 2014"
#define PROTOCOL_NAME "bw6"
#define BITS_IN_WORLD 167
#define HASHC
...
class mu_1_block: public mu__byte
{
 public:
  ...
  mu_1_block (const char *name, int os): mu__byte(1, 6, 3, name, os) {};
  mu_1_block (void): mu__byte(1, 6, 3) {};
  mu_1_block (int val): mu__byte(1, 6, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
const char *mu_1_block::values[] = {"b1","b2","b3","b4","b5","b6",NULL };
...
friend int Compare(mu_1__type_0& a, mu_1__type_0& b)
  ...
  std::vector<mu_0_boolean*> bool_array() {
	...
	#ifdef awesome_mu_00_mu_0_boolean_mu_1__type_0
		for (int ix = 0; ix < 6; ix++){ barr.push_back(&(array[ix])); }
		return barr;
	#else 
		for (int ix = 0; ix < 6; ix++){
			for (int ix2 = 0; ix2 < 6; ix2++){ barr.push_back(&(array[ix][ix2+1])); }
		}
		return barr;
...
if (n) array[i].set_self_ar(n,"b1", i * 12 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"b2", i * 12 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"b3", i * 12 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"b4", i * 12 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"b5", i * 12 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
if (n) array[i].set_self_ar(n,"b6", i * 12 + os); else array[i].set_self_ar(NULL, NULL, 0); i++;
  }
...
/*** Variable declaration ***/
mu_1__type_2 mu_ontable("ontable",129);

/*** Variable declaration ***/
mu_1__type_3 mu_isclear("isclear",141);

/*** Variable declaration ***/
mu_0_boolean mu_handempty("handempty",153);

/*** Variable declaration ***/
mu_1__type_4 mu_holding("holding",155);
...
public:
  ...
  void NextRule(RULE_INDEX_TYPE & what_rule)
  {
    RULE_INDEX_TYPE r = what_rule - 73;
    static mu_1_block mu_x;
    mu_x.value((r % 6) + 1);
    r = r / 6;
    while (what_rule < 79 )
...
bool mu__boolexpr14;
  ...
  void NextRule(RULE_INDEX_TYPE & what_rule)
  {
    RULE_INDEX_TYPE r = what_rule - 79;
    static mu_1_block mu_x;
    mu_x.value((r % 6) + 1);
    r = r / 6;
    while (what_rule < 85 )
...
bool Condition(RULE_INDEX_TYPE r)
{
  ...
  if (r>=1 && r<=36) return R1.Condition(r-1);
  if (r>=37 && r<=72) return R2.Condition(r-37);
  if (r>=73 && r<=78) return R3.Condition(r-73);
  if (r>=79 && r<=84) return R4.Condition(r-79);
...
std::vector<mu_0_boolean*> precond_array(RULE_INDEX_TYPE r)
{
  ...
  if (r>=1 && r<=36) return R1.precond_array(r-1);
  if (r>=37 && r<=72) return R2.precond_array(r-37);
  if (r>=73 && r<=78) return R3.precond_array(r-73);
  if (r>=79 && r<=84) return R4.precond_array(r-79);
Error.Notrace("Internal: NextStateGenerator -- checking preconditions for nonexisting rule.");
}
std::vector<mu_0_boolean*> effects_array(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.effects_array(r-0);
  if (r>=1 && r<=36) return R1.effects_array(r-1);
  if (r>=37 && r<=72) return R2.effects_array(r-37);
  if (r>=73 && r<=78) return R3.effects_array(r-73);
  if (r>=79 && r<=84) return R4.effects_array(r-79);
Error.Notrace("Internal: NextStateGenerator -- checking effects for nonexisting rule.");
}
std::vector<mu_0_boolean*> effects_add_array(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.effects_add_array(r-0);
  if (r>=1 && r<=36) return R1.effects_add_array(r-1);
  if (r>=37 && r<=72) return R2.effects_add_array(r-37);
  if (r>=73 && r<=78) return R3.effects_add_array(r-73);
  if (r>=79 && r<=84) return R4.effects_add_array(r-79);
Error.Notrace("Internal: NextStateGenerator -- checking add effects for nonexisting rule.");
}
void Code(RULE_INDEX_TYPE r)
{
  if (r<=0) { R0.Code(r-0); return; } 
  if (r>=1 && r<=36) { R1.Code(r-1); return; } 
  if (r>=37 && r<=72) { R2.Code(r-37); return; } 
  if (r>=73 && r<=78) { R3.Code(r-73); return; } 
  if (r>=79 && r<=84) { R4.Code(r-79); return; } 
}
void Code_ff(RULE_INDEX_TYPE r)
{
  if (r<=0) { R0.Code_ff(r-0); return; } 
  if (r>=1 && r<=36) { R1.Code_ff(r-1); return; } 
  if (r>=37 && r<=72) { R2.Code_ff(r-37); return; } 
  if (r>=73 && r<=78) { R3.Code_ff(r-73); return; } 
  if (r>=79 && r<=84) { R4.Code_ff(r-79); return; } 
}
int Priority(RULE_INDEX_TYPE r)
{
  if (r<=0) { return R0.Priority(); } 
  if (r>=1 && r<=36) { return R1.Priority(); } 
  if (r>=37 && r<=72) { return R2.Priority(); } 
  if (r>=73 && r<=78) { return R3.Priority(); } 
  if (r>=79 && r<=84) { return R4.Priority(); } 
}
char * Name(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.Name(r-0);
  if (r>=1 && r<=36) return R1.Name(r-1);
  if (r>=37 && r<=72) return R2.Name(r-37);
  if (r>=73 && r<=78) return R3.Name(r-73);
  if (r>=79 && r<=84) return R4.Name(r-79);
  return NULL;
}
int Duration(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.Duration(r-0);
  if (r>=1 && r<=36) return R1.Duration(r-1);
  if (r>=37 && r<=72) return R2.Duration(r-37);
  if (r>=73 && r<=78) return R3.Duration(r-73);
  if (r>=79 && r<=84) return R4.Duration(r-79);
Error.Notrace("Internal: NextStateGenerator -- querying duration for nonexisting rule.");
}
int Weight(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.Weight(r-0);
  if (r>=1 && r<=36) return R1.Weight(r-1);
  if (r>=37 && r<=72) return R2.Weight(r-37);
  if (r>=73 && r<=78) return R3.Weight(r-73);
  if (r>=79 && r<=84) return R4.Weight(r-79);
Error.Notrace("Internal: NextStateGenerator -- querying duration for nonexisting rule.");
}
char * PDDLName(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.PDDLName(r-0);
  if (r>=1 && r<=36) return R1.PDDLName(r-1);
  if (r>=37 && r<=72) return R2.PDDLName(r-37);
  if (r>=73 && r<=78) return R3.PDDLName(r-73);
  if (r>=79 && r<=84) return R4.PDDLName(r-79);
  return NULL;
}
RuleManager::rule_pddlclass PDDLClass(RULE_INDEX_TYPE r)
{
  if (r<=0) return R0.PDDLClass(r-0);
  if (r>=1 && r<=36) return R1.PDDLClass(r-1);
  if (r>=37 && r<=72) return R2.PDDLClass(r-37);
  if (r>=73 && r<=78) return R3.PDDLClass(r-73);
  if (r>=79 && r<=84) return R4.PDDLClass(r-79);
Error.Notrace("Internal: NextStateGenerator -- querying PDDL class for nonexisting rule.");
}
};
const RULE_INDEX_TYPE numrules = 85;
...
public:
  ...
  {
{
for(int mu_x = 1; mu_x <= 6; mu_x++) {
{
for(int mu_y = 1; mu_y <= 6; mu_y++) {
...
mu_fun = 0.000000e+00;
mu_isclear[mu_b3] = mu_true;
mu_isclear[mu_b5] = mu_true;
mu_isclear[mu_b6] = mu_true;
mu_ontable[mu_b1] = mu_true;
mu_ontable[mu_b2] = mu_true;
mu_ontable[mu_b4] = mu_true;
mu_on[mu_b3][mu_b1] = mu_true;
mu_on[mu_b5][mu_b4] = mu_true;
mu_on[mu_b6][mu_b2] = mu_true;
mu_handempty = mu_true;
mu_h_n = 0;
mu_g_n = 0;
...
bool mu__boolexpr20;
bool mu__boolexpr21;
  if (!(mu_on[mu_b1][mu_b4])) mu__boolexpr21 = FALSE ;
  else {
  mu__boolexpr21 = (mu_on[mu_b2][mu_b6]) ; 
}
  if (!(mu__boolexpr21)) mu__boolexpr20 = FALSE ;
  else {
  mu__boolexpr20 = (mu_on[mu_b3][mu_b5]) ; 
}
  if (!(mu__boolexpr20)) mu__boolexpr19 = FALSE ;
  else {
  mu__boolexpr19 = (mu_on[mu_b5][mu_b1]) ; 
}
  if (!(mu__boolexpr19)) mu__boolexpr18 = FALSE ;
  else {
  mu__boolexpr18 = (mu_on[mu_b6][mu_b3]) ; 
}
return mu__boolexpr18;
};

bool mu__condition_22() // Condition for Rule "enjoy"
{
  return mu__goal_17( );
}

bool mu__goal__00(){ return mu__condition_22(); } /* WP WP WP GOAL CONDITION CHECK */ /**** end rule declaration ****/

const rulerec goals[] = {
{"enjoy", &mu__condition_22, NULL, },
};
